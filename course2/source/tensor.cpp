...
void Tensor<float>::Padding(const std::vector<uint32_t>& pads,
  ...
  for (uint32_t channel = 0; channel < pad_channels; channel ++) {
    for (uint32_t row = pad_rows1; row < pad_rows1 + this->rows(); row ++) {
      for (uint32_t col = pad_cols1; col < pad_cols1 + this->cols(); col ++) {
        float& item = padded.at(channel, row, col);
        item = this->at(channel, row - pad_rows1, col - pad_cols1);
      }
    }
  }
  this->data_ = padded.data_;
  this->raw_shapes_ = padded.raw_shapes_;
}
...
void Tensor<float>::Flatten(bool row_major) {
  ...
  uint32_t total_elems = this->data_.size();
